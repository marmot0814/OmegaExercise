...
bigN solve(LL x, LL y, vector<vector<bigN> > &dp, vector<pii> &data, LL c, LL d) {
    if (x <= 0 || y <= 0) return d;
    if (dp[x][y] != -1) return dp[x][y];
    dp[x][y] = c;
    for (auto &p : data)
        dp[x][y] = dp[x][y] + solve(x - p.first, y - p.second, dp, data, c, d);
    return dp[x][y];
}
int main() {
    ...
    int t; cin >> t; string input; getline(cin, input); while (t--) {
        ...
        LL x, y; while (ss >> x >> y)
            data.push_back({x, y});
        LL c = data.back().first, d = data.back().second;
        data.pop_back();
        getline(cin, input);
        ss.clear(); ss << input;
        vector<vector<bigN> > dp(100, vector<bigN>(100, -1));
        while (ss >> x >> y)
            cout << solve(x, y, dp, data, c, d) << '\n';
        cout << '\n';
    }
}
